...
const TeacherNavbar = ({ currentPage, onNavigate }) => {
  const { user, logout } = useAuth()
  const [showNotifications, setShowNotifications] = useState(false)
  const [showUserMenu, setShowUserMenu] = useState(false)

  const navItems = [
    { id: 'tasks', label: 'Tasks', icon: 'ph ph-clipboard-text' },
        ...
        <div className="nav-container">
          ...
          <div className="nav-user" style={{ gap: '12px' }}>
            ...
            <div className="user-avatar" onClick={handleUserClick}>
              <img
                src={user?.avatar || "https://via.placeholder.com/40x40/4CAF50/FFFFFF?text=T"}
        ...
        <div className="user-menu">
          <div className="user-menu-content">
            <div className="user-info">
              <img src={user?.avatar || "https://via.placeholder.com/40x40/4CAF50/FFFFFF?text=T"} alt={user?.name || "Teacher"} />
              <div>
                <h4>{user?.name || "Teacher"}</h4>
                <p>{user?.role === 'teacher' ? 'Educator' : user?.role || "Teacher"}</p>
                <span className="user-id">ID: {user?.id || 'T001'}</span>
              </div>
            </div>
